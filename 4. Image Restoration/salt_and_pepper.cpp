			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) { //if the pixel is not a border pixel
							kernel.at<G>(kernel_num++, 0) = input.at<G>(i + x, j + y);
						}
						else
							kernel_num++;
					}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(kernel_num++, 0) = input.at<G>(tempx, tempy);
					}
				}
			}
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						// Median filter with "zero-padding" boundary process:
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) { //if the pixel is not a border pixel
							kernel.at<G>(kernel_num, 0) = input.at<C>(i + x, j + y)[0];
							kernel.at<G>(kernel_num, 1) = input.at<C>(i + x, j + y)[1];
							kernel.at<G>(kernel_num, 2) = input.at<C>(i + x, j + y)[2];
							kernel_num++;
						}
						else {
							kernel_num++;
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(kernel_num, 0) = input.at<C>(tempx, tempy)[0];
						kernel.at<G>(kernel_num, 1) = input.at<C>(tempx, tempy)[1];
						kernel.at<G>(kernel_num, 2) = input.at<C>(tempx, tempy)[2];
						kernel_num++;
					}
				}
			}
