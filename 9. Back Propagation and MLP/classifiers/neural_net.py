...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    fc1 = X.dot(W1) + b1       # first fully connected network
    X2 = np.maximum(0, fc1)    # ReLU function
    scores = X2.dot(W2) + b2   # get scores (S = Wx + b) --> second fully connected network
    ...
    scores_exp = np.exp(scores)   # use exponential function
    softmax_matrix = scores_exp / np.sum(scores_exp, axis=1, keepdims=True)  # softmax formula
    loss = np.sum(-np.log(softmax_matrix[np.arange(N), y]))  # get loss (log likelihood loss)
